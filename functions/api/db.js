...
export async function onRequestGet(context) {
  ...
  // Ensure all default users exist in KV
  for (const email of Object.keys(defaultDb.users)) {
    if (!db.users[email]) {
      db.users[email] = defaultDb.users[email];
      needsUpdate = true;
    }
  }
...
export async function onRequestPost(context) {
  ...
  // Ensure default users exist in KV
  for (const email of Object.keys(defaultDb.users)) {
    if (!currentDb.users[email]) {
      currentDb.users[email] = defaultDb.users[email];
      needsUpdate = true;
    }
  }
